        ...
        with nogil:
            while not stack.is_empty():
                ...
                if depth >= max_depth 
                    ...
                    node_id = tree._add_node(
                            parent,
                            index,           # the new node's index
                            False,     # not leaf node
                            split_record.feature_index,
                            split_record.threshold,
                            split_record.n_values,  # number of children       333
                            n_node_samples, 
                            weighted_n_node_samples #XXX
                            )
...
#         node.impurity = impurity
        ...
        node.n_children = n_children ########################3333
        node.index_children = 0

        if is_leaf:
            node.children = NULL
        else:
            node.children = <SIZE_t*> calloc(n_children+1, sizeof(SIZE_t))
            

        if parent != _TREE_UNDEFINED:
            if(self.nodes[parent].index_children < n_children)
                self.nodes[parent].children[self.nodes[parent].index_children]=node_id
                self.nodes[parent].index_children+=1
